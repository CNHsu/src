...
import os
root = tk.Tk()
stitle='BES key Test TOOL 2019/8/6'
root.title(stitle)
...
def bt():
    text_output.insert(tk.END,"\n[3S] press [power/V+/V-/ANC] then [start]: ")
    root.update()
    #f=-1
    #while f ==-1:
    print('power key')
    while ser.in_waiting:# 若收到序列資料…
            ...
            f=st1.find(sfind)
            if f !=-1:
                print("got key word 0!!")
                ssfind="\n[3S] "+' '+'find first power key'
                text_output.insert(tk.END,ssfind)
            if st1.find("app_bt_key 1,5") != -1:
                print("got key word power key!!")
                ssfind="\n[3S] "+' '+'find power key'
                text_output.insert(tk.END,ssfind)
            if st1.find("app_bt_key 4,2") != -1:
                print("got key word V- !!")
                ssfind="\n[3S] "+' '+'find V-'
                text_output.insert(tk.END,ssfind)                         
            if st1.find("app_bt_key 2,2") != -1:
                print("got key word V+ !!")
                ssfind="\n[3S] "+' '+'find V+'
                text_output.insert(tk.END,ssfind)
            if st1.find("app_anc_key") != -1:
                print("got key word ANC !!")
                ssfind="\n[3S] "+' '+'find ANC'
                text_output.insert(tk.END,ssfind)                 
            #else:
                 #print('1>>接收到的資料：', st1)
