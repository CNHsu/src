...
{ 
 ...
 ////////////////////////////////////
 // https://ir.nctu.edu.tw/bitstream/11536/30400/1/000088843800008.pdf
 // IEEE booth paper
 // qr[2i+1] qr[2i] qr[2i-1]
 //
 //////////////////////////////////// 
 
void boothIEEE(int br[], int qr[], int mt[], int qrn, int sc) 
{ 
    ...
    display(ac, qr, qrn); 
    cout << "\t\t\t" << sc << "\n"; 
     for(k=0,i=0;k<qrn;k=i*2){
    	 if(k==0) qr0=0; else qr0=qr[k-1];     	    	
    	if( qr0==0 && qr[k]==1 && qr[k+1]==0) printf("\n\r+0");
    	else if(qr0==0 && qr[k]==1 && qr[k+1]==1) printf("0+" );
    	else if(qr0==0 && qr[k]==0 && qr[k+1]==1) printf("0-" );
    	else if(qr0==1 && qr[k]==0 && qr[k+1]==0) printf("+0" );
    	else if(qr0==1 && qr[k]==1 && qr[k+1]==0) printf("0+" );
    	else if(qr0==1 && qr[k]==0 && qr[k+1]==1) printf("-0" );
    	else printf("00");
    	i++;    	
    }     
    cout << endl;
...
printf("\n");
//reverse(qr, qr + *num); // need for booth int part, qr need reverse
                          // but not reverse for IEEE
...
{ 
    ...
    int mt[20], sc;                              // mt=(-BR) is 2's of BR for A-BR
