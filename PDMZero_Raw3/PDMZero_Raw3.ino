...
PDMZero Mypdm = PDMZero(1, 9);   // Create PDM object, with Clock and Data pins used clock=> PA10 = ~2  data => PA07 = A6  MKR1000 / MKR1010
                                 // arduino zero I2S clk:D1, dat: D9
...
int t=10;  //Initialize serial and wait for port to open, max 10 second waiting
  ...
  SerialUSB.begin(1000000); // native serial
  Serial.begin(921600); // arduino wemos...on board connect RXD, TXD, not list on GPIO
  //Serial.begin(921600); // arduino wemos...board connect D0,D1
  while (!SerialUSB) { // arduino has build in 6 soft serial
     delay(1000);
    if ( (t--)== 0 ) break;
  }
...
/*********** PDM over I2S Setup ************/
if (!Mypdm.begin()) {
    SerialUSB.println("Failed to initialize I2S/PDM!");
    while (1);
   }
if (!Mypdm.configure(SAMPLERATE_HZ * DECIMATION/16, true)) {
    SerialUSB.println("Failed to configure PDM");
    while (1);
  }
SerialUSB.println("PDM Test starts");             
}
...
{
 if (samples[0][x]!=0) { 
  SerialUSB.print(68000); SerialUSB.print(",");                      // autorange max manual -> Plotter
  SerialUSB.print(64000); SerialUSB.print(",");      
  a=(samples[0][x] >>6) & 0x00ff;
  b=((samples[0][x] >>6)>>8) & 0x007f;  
  Serial.write(a);Serial.write(b); // write log                 
  SerialUSB.print(2*samples[0][x]-samples[1][x]); SerialUSB.print(","); //Serial.println(samples[0][x]);Serial.print("(");Serial.write(a);Serial.write(b);Serial.print(")");
  SerialUSB.print(2*samples[1][x]-samples[0][x]); SerialUSB.println(",");
 } // end inf not zero
}
}
