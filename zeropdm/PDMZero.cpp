// Arduino Zero I2S audio library.
// Author: Jay fox
//
// The MIT License (MIT)
//
...
PDMZero::PDMZero(int clockpin, int datapin, uint8_t gclk):
  _gclk(gclk), _clk(clockpin), _data(datapin)  {

}

bool PDMZero::begin(void) {
...
bool PDMZero::configure(uint32_t sampleRateHz, boolean stereo) {
  ...
  /***************************** Configure I2S serializer *************/
  {
    ...
    uint32_t serctrl =
      // I2S_SERCTRL_RXLOOP |    // NEver use loopback mode
      // I2S_SERCTRL_DMA    |    // Single DMA channel for all I2S channels
      // I2S_SERCTRL_MONO   |    // Dont use MONO mode - always Stereo
      ...
      I2S_SERCTRL_BITREV   |  // Transfer MSB first, as Decimator needs a shift-through function into FIR Filter
    ...
    /* Initialize pins */
    // Enable SD pin.  See ZeroI2S.h for default pin value.
