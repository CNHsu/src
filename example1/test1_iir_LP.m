close all;
clear all;
clf;


f1 = 1000; %low pass@11200Hz
f2 = 1500;%stop band @ 15K
delta_f = f2-f1;% transition bandwitdh
Fs = 48000; %sample rate 192K
dB  = 30; %stop band attenuation
N = dB*Fs/(22*delta_f); % filter length
Rpass= 1 %pass band ripple
Rstop= 15 %stop band ripple
Wpass=2/Fs*f1;
Wstop=2/Fs*f2;

f =  [f1 ]/(Fs/2)
% use ellip to design the filter
%hc = fir1(round(N)-1, f,'low')
[n,Wn]=ellipord(Wpass, Wstop,Rpass,Rstop); % get the minimum order n
n
[b,a]=ellip(n,Rpass, Rstop, Wn);
b
a
 f = 0:Fs/2;
 W = f * (2 * pi / Fs);
 H = freqz (b, a, W);
 %semilogx (f, 20 * log10 (abs (H))) % for semilogx may need change f=1000:Fs/2 range
 plot (f, 20 * log10 (abs (H)));
 outline_lp_pass_x = [f(2)  , f1(1), f1(1)];
 outline_lp_pass_y = [-Rpass, -Rpass  , -80];
 outline_lp_stop_x = [f(2)  , f2(1), f2(1), max(f)];
 outline_lp_stop_y = [0     , 0       , -Rstop  , -Rstop];
 hold on
 plot (outline_lp_pass_x, outline_lp_pass_y, "m", outline_lp_stop_x, outline_lp_stop_y, "m");
 ylim ([-80, 0],'color','red');
 grid on
 xlabel ("Frequency (Hz)");
 ylabel ("Attenuation (dB)");
 title ("2nd order digital elliptical low-pass (without margin)");
 
 ##x = sin(2*pi*[1:1000]*5000/Fs) +  sin(2*pi*[1:1000]*2000/Fs) + sin(2*pi*[1:1000]*13000/Fs)  + sin(2*pi*[1:1000]*18000/Fs);
x = sin(2*pi*[1:1000]*1000/Fs) +  sin(2*pi*[1:1000]*2000/Fs) + sin(2*pi*[1:1000]*13000/Fs)  + sin(2*pi*[1:1000]*18000/Fs);
 
 
 y=filter(b,a,x);
figure
subplot(211)
plot(x)
title('Sinusoid with frequency components 1000, 2000, 13000, and 18000 Hz')
subplot(212)
plot(y)
title('Filtered Signal')
xlabel('time')
ylabel('amplitude')

figure
x= (x/sum(x))/20
y=filter(b,a,x);
sig = 20*log10(abs(fftshift(fft(x,4096))));
subplot(211)
plot((-0.5:1/4096:0.5-1/4096)*Fs,sig)
hold on
plot(f, 20 * log10 (abs (H)),'color','r')
hold off
axis([0 20000 -60 10])
title('Input to filter - 4 Sinusoids')
grid on
subplot(212)
plot((-0.5:1/4096:0.5-1/4096)*Fs,20*log10(abs(fftshift(fft(y,4096)))))
axis([0 20000 -60 10])
title('Output from filter')
xlabel('Hz')
ylabel('dB')
grid on



