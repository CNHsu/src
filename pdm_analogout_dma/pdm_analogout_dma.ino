// 2020/11/13 CN modified
// this code has good result to play back
...
Adafruit_ZeroPDM pdm = Adafruit_ZeroPDM(1, 9);      // Metro M0 or Arduino zero use 1,9
...
void setup() {
  ...
  SerialUSB.println("SAMD PDM DMA Demo");
  ...
  SerialUSB.println("PDM initialized");
  ...
  SerialUSB.println("PDM configured");
...
void loop() {
 ...
 //CN mark>>> analogWrite(A0, runningsum);

  runningsum /= 16 ; // /64 10bit, /32 11 bit(ok), /16 12bit(ok), /8 13bit(not good), /4 14bit(bad), /2 15bit(bad)
  a=runningsum & 0x00ff;
  b=(runningsum >>8 ) & 0x007f;  
