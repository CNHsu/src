  ...
  */
  ...
  switch (i){
   ...
   case 'i'  :Serial.print("**K3=0\n\r");
             ...
             digitalWrite(LED3,  LOW);
             break ;           
   case 'u'  : // initial HC-05
              Serial.print("**'u'-BT INIT\n\r");
              Serial.write("AT+RESET\n\r ");
              BTSerial.print("AT+RESET\n\r ");
             btrx = BTSerial.readString();
             while(btrx != "OK")  // read BT wait 'ok'
               {                 
                btrx = BTSerial.readString();
                Serial.println(btrx);
               } 
             ...
             while(btrx != "OK\n\r")  // read BT wait 'ok'
              ...
              Serial.println(btrx); // out to UART                                           
             break ;  
   case 'v'  :Serial.print("**BT scan\n\r"); // BT scan
             BTSerial.print("AT+INQ\n\r ");
             btrx=" ";
             Rssi="";
             while (btrx.startsWith(" ",0)){
               if(BTSerial.available())  // read BT wait 'ok'
               {                 
                btrx = BTSerial.readString();
                Serial.print(btrx);
                Rssi = btrx.substring( btrx.length()-6,btrx.length()-2);
               } 
                Serial.println(btrx); // out to UART
                Serial.println(Rssi);
             }            
